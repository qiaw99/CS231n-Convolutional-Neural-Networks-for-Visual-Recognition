...
def softmax_loss_naive(W, X, y, reg):
    ...
    # Reason: https://stackoverflow.com/questions/41663874/cs231n-how-to-calculate-gradient-for-softmax-loss-function
    for i in range(num_train):
        all_sum = np.sum(np.exp(scores[i,:]))
        y_i = np.exp(scores[i, y[i]])
        loss += -np.log(y_i / all_sum)
